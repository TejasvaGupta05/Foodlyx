...
const DEMO_ACCOUNTS = [
  {
    ...
    address: '123 Donor Street, New Delhi',
    contactPhone: '9876543210',
    donorCategory: 'party_gathering',
    impactScore: 320,
  },
  {
  ...
  },
  {
    ...
    address: '789 Paws Road, Bangalore',
  ...
  },
  {
    ...
    address: '101 Green Hub, Hyderabad',
  ...
  },
  {
    ...
    role: 'admin',
    location: { lat: 28.6139, lng: 77.2090 },
    address: '000 Admin HQ, New Delhi',
...
export default function SeedDemoAccounts() {
  ...
  const runSeed = async () => {
    ...
    for (const account of DEMO_ACCOUNTS) {
      const { email, password, name, role, location, impactScore, address, contactPhone, donorCategory } = account;
      try {
      ...
      <div className="w-full max-w-lg glass p-8 glow fade-in">
        ...
        {results.length > 0 && (
          <div className="mb-6 space-y-2">
            {results.map((r, i) => (
              <div key={i} className={`flex items-center gap-2 p-3 rounded-lg text-sm ${r.status === 'success' ? 'bg-green-500/10 border border-green-500/20 text-green-300' :
